...
{
    printf("AppEIS_RegisterObject" NEWLINE);
...
uint32_t AppEIS_ConfigureStack(APP_DATA_T* ptAppData)
{
    ...
    for (ulIdx = 0; ulIdx < s_ulCipObjDirEntries; ulIdx++) {
        printf("AppEIS_SetAttributeSingle" NEWLINE);
...
static void AppEIS_HandleCl3Ind(APP_DATA_T* ptAppData)
{
    ...
    printf("Ind.Len=%d" NEWLINE, ptRes->tHead.ulLen);
    printf("Res.Len=%d" NEWLINE, EIP_OBJECT_CL3_SERVICE_RES_SIZE);

    ptRes->tData.ulGRC = CIP_GSR_SUCCESS;
    ptRes->tData.ulERC = 0;
    ptRes->tData.abData[0] = 0xD;
    ptRes->tHead.ulLen = EIP_OBJECT_CL3_SERVICE_RES_SIZE + 1;
    ...
    void* pvUserData)
{
    ...
    case HIL_STORE_REMANENT_DATA_IND:
        printf("HIL_STORE_REMANENT_DATA_IND" NEWLINE);
