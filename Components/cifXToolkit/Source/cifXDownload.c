...
***************************************************************************************
  ...
  $Id: cifXDownload.c 12740 2019-02-22 11:16:30Z LuisContreras $:
...
{
  /* Check if we have a .NXI or .NAI extension, or FWUPDATE.ZIP
  ...
  */
  ...
  if( iStrlen >= CIFX_MIN_FILE_NAME_LENGTH)  /* At least x.abc */
  {
    ...
    else if( iStrlen == OS_Strlen("FWUPDATE.ZIP"))  /* Check for update container */
    {
      if ( (0 == OS_Strnicmp( "FWUPDATE.ZIP", &pszFileName[0], 12) )  )
      {
        fRet = 1;
      }
    }
